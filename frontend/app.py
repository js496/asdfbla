...
def docker_api_create(req_model, req_pipeline_tag, req_port_model, req_port_vllm):
    try:
        ...
        response = requests.post(BACKEND_URL, json={"req_method":"create","req_container_name":req_container_name,"req_model":req_model,"req_runtime":"nvidia","req_port_model":req_port_model,"req_port_vllm":req_port_vllm})
...
def calculate_model_size(json_info):
    try:
...
with gr.Blocks() as app:
    ...
    info_textbox = gr.Textbox(value="Interface not supported check 'pipeline_tag', 'transformers', 'private', 'gated'", show_label=False, visible=False)
    ...
    timer_dl_box2 = gr.Textbox(label="Total download speed:", visible=False)
    ...
    def show_split(text_pipeline, text_model):
        if len(text_model) == 0:
            ...
            try:
                ...
                gr.Interface.from_pipeline(pipeline(text_pipeline, model=text_model,tokenizer=tokenizer,device=0))
            except Exception as e:
                logging.exception(f'Exception occured: {e}', exc_info=True)
    ...
    def render_container(render_container_list):
        ...
        gr.Markdown(f'### Bedrock Container ({len(docker_container_list_not_running)})')
    ...
    timer_dl.tick(docker_api, ['network',create_response], timer_dl_box)
    
    timer_dl2 = gr.Timer(1,active=False)
    timer_dl2.tick(get_total_download_speed, outputs=timer_dl_box2)
    
    btn_dl.click(lambda: gr.update(label="Building vLLM container",visible=True), None, create_response).then(lambda: gr.update(visible=True), None, timer_dl_box).then(lambda: gr.update(visible=True), None, timer_dl_box2).then(lambda: gr.Timer(active=True), None, timer_dl).then(lambda: gr.Timer(active=True), None, timer_dl2).then(docker_api_create,inputs=[model_dropdown,selected_model_pipeline_tag,port_model,port_vllm],outputs=create_response).then(refresh_container_list, outputs=[container_state]).then(lambda: gr.update(visible=True), None, btn_interface).then(lambda: gr.Timer(active=False), None, timer_dl).then(lambda: gr.Timer(active=False), None, timer_dl2)
